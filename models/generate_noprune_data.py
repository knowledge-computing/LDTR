...
def patch_extract(save_path, image, seg, mesh, device=None):
    ...
    h, w, d = image.shape
    x_ = np.int32(np.linspace(5, h-5-p_h, 12)) # bray wt pos: 40, louis wt pos: 20, bray wt neg: 8, louis wt neg 12
    y_ = np.int32(np.linspace(5, w-5-p_w, 12)) # bray wt pos: 40, louis wt pos: 20, bray wt neg: 8, louis wt neg 12
...
#                 if np.sum(patch_seg/255) < 1200 or (xmax-xmin < 0.7 and ymax-ymin < 0.7) or\
...
def conflate_nodes(patch_coord_list, patch_edge_list):
    ...
    for coord, edge in zip(patch_coord_list, patch_edge_list):
        ...
        dup_node_pairs = np.array(np.where((dist_all<0.0015))).T
        dup_node_pairs.sort()
...
def prune_patch(patch_coord_list, patch_edge_list):
    ...
    for coord, edge in zip(patch_coord_list, patch_edge_list):
        ...
        for i in range(coord.shape[0]):
            if np.sum(dist_adj[i]) == 0: # node degree = 0
                node_mask[i]=False
        ...
        while start:
            ...
            for n, idx in enumerate(deg_2):
                ...
                if node_angle > 160 and (dist1 < 0.04 or dist2 < 0.04 or dist1+dist2 < 0.2):
                    node_mask[idx]=False
...
# indrange_train = [i for i in range(42) if i not in [26,37,8,15,22,38]]
# indrange_train = [i for i in range(30) if i not in [27, 6, 16]]
indrange_train = [i for i in range(24) if i not in [5, 7, 11]] # louiville wt
# indrange_train = [i for i in range(42) if i not in [10,13,16,32,34,8]] # bray wt
# indrange_train = [19]
indrange_test = []
...
#     root_dir = "/data/weiweidu/sat2graph_inputs/CA_Bray_2001_waterlines"
    ...
    image_id = 5998 # 1896, #pos for louis wt, 3818 #pos for bray+louis wt, 5994
    train_path = './data/usgs/waterlines/train_data_g256_update/'
...
#         vtk_files.append(root_dir + "/ca_bray_2001_waterlines_region%d.p" % ind)
    for ind in indrange_train:
        raw_files.append(root_dir + "/co_louisville_1965_waterlines_region%d" % ind)
        seg_files.append(root_dir + "/co_louisville_1965_waterlines_gt_region%d.png" % ind)
        vtk_files.append(root_dir + "/co_louisville_1965_waterlines_region%d.p" % ind)

    for ind in range(len(raw_files)):
        print(ind)
        try:
            print(raw_files[ind], os.path.isfile(raw_files[ind]+".png"))

...
def patch_extract(save_path, image, mask, seg, mesh, device=None):
    """[summary]
    ...
    x_ = np.int32(np.linspace(5, h-5-p_h, 16)) # pos: 128 (thrust), neg: 32 (thrust), pos: 256 (scarp), neg: 16/25 (scarp)
    # pos: 100 (thrust512)
    y_ = np.int32(np.linspace(5, w-5-p_w, 16)) # pos: 128 (thrust), neg: 32 (thrust), pos: 256 (scarp), neg: 16/25 (scarp)
...
#         patch = np.pad(image[start[0]:start[0]+p_h, start[1]:start[1]+p_w, :], ((pad_h,pad_h),(pad_w,pad_w),(0,0)))
        ...
        patch_mask = np.pad(mask[start[1]:start[1]+p_h, start[0]:start[0]+p_w,], ((pad_h,pad_h),(pad_w,pad_w)))
...
# indrange_train = [i for i in range(30)]
# indrange_train = [28,30]
indrange_test = []

if __name__ == "__main__":
    root_dir = "/data/weiweidu/data/relationform/thrust_fault_lines/sat2graph_train"

    image_id = 2404 # 5921 #pos512 for thrust, 5696 #pos256 for thrust, #2008 for scarp
    train_path = './data/darpa/thrust_fault_lines/train_data_g512_update/'
...
#                 if 'scarp_line' in f and 'png' == fmt and name in f:
    ...
    for ind in range(len(raw_files)):
        ...
        print('mask img: ', mask_img.shape, np.unique(mask_img))
...
#         print('patch_coord, patch_edge shape: ', patch_coord.shape, patch_edge.shape)
        mesh.lines = patch_edge.flatten()
        patch_extract(train_path, sat_img, mask_img, gt_seg, mesh)
